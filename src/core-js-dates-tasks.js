...
function getDayName(date) {
  const today = new Date(date);
  const getDayResult = today.getUTCDay();
  const dayArray = [
    'Sunday',
    'Monday',
    'Tuesday',
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return (end - start) / MS_IN_ONE_DAY + 1;
}
...
function isDateInPeriod(date, period) {
  ...
  const emd = new Date(period.end);

  const targetDate = new Date(date);

  const startTime = start.getTime();
  const endTime = emd.getTime();
...
function isLeapYear(date) {
  ...
  const februaryLastDay = new Date(year, 2, 0);
  return februaryLastDay.getDate() === 29;
}
